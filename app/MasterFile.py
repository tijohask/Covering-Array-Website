...
def create_test_array(array, size):
  ...
  #Generate the indexes we want to test
  ii = sorted(random.sample(range(array.shape[0]), size))
  #and return the array with just those indexes
  return array[ii,:]
...
def create_cover(k_level, num_inter):
  ...
  #get the precataloged covering array size
...
def main():
  ...
  give = switch(k_level, num_inter, check)
  print(give)
  catalog["covering array"] = str(give)
  ...
  catalog["columns"] = give.shape[1]
  catalog["rows"] = give.shape[0]
  #catalog["possible rows"] = total
  
  #find and print the time elapsed
  end = time.process_time()
  print("Elapsed Time: " + str(end - start))
  catalog["time"] = str(end-start)
  
  print(catalog)

#Choose which of the algorithms to use based on the third argument.
def switch(k_level, num_inter, check):
